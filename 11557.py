...
for _ in range(int(input())): # 테스트 케이스 수를 입력 받고 반복함
    
    # 변수 선언
    name, max = '', 0


    for _ in range(int(input())): # 학교 수를 입력 받고 반복함

        x, y = map(str, input().split()) # 학교명, 술 소비량을 띄어쓰기를 기준으로 분할하여 변수에 값 저장함
        y = int(y)

        # 두 값을 비교하고 최댓값을 출력
        if y > max: # 소비량은 정수로 저장
            name = x
            max = y
